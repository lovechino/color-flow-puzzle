/**
 * Generate new levels by mutating existing valid levels
 * This GUARANTEES solvability because we start from a known-solvable configuration
 * 
 * Run: npx tsx scripts/generate-from-seeds.ts
 */
...
function mutateLevel(seedLevel: LevelData, rng: SeededRandom, mutationCount: number): LevelData | null {
  ...
  // Try multiple mutations: swap dots, shift positions
  for (let m = 0; m < mutationCount; m++) {
    const mutationType = rng.nextInt(3);

    if (mutationType === 0) {
      // Swap position of 2 random dots (same color)
      if (pairs.length >= 2) {
      ...
      const i = rng.nextInt(pairs.length);
      const dotIdx = rng.nextInt(2); // start or end
      ...
      if (nr >= 0 && nr < gridSize && nc >= 0 && nc < gridSize) {
        // Check not overlapping with other dots
        const occupied = new Set<string>();
        pairs.forEach((p, idx) => {
          if (idx === i && dotIdx === 1) return; // same color's other dot OK
          if (idx === i && dotIdx === 0) return;
  ...
  // Try to solve with mutated pairs
...
async function main() {
  console.log('🧬 Generating levels from existing seeds...\n');
  
  const gridSize = 5;
  const targetCount = 10; // We need 10 total for 5x5
  ...
  for (const seedLevel of existingLevels) {
    ...
    for (let mutationCount = 1; mutationCount <= 200; mutationCount++) {
      ...
      mutated.id = `g05_${String(nextIndex).padStart(3, '0')}`;
  ...
  if (generated < needed) {
    console.log(`⚠️  Could only generate ${generated} more levels`);
    console.log(`   Consider reducing total level count or hand-crafting remaining levels`);
  } else {
